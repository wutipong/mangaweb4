...
func (_q *UserQuery) QueryFavoriteItems() *MetaQuery {
...
func (_q *UserQuery) QueryFavoriteTags() *TagQuery {
...
func (_q *UserQuery) QueryProgress() *ProgressQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFavoriteItems: _q.withFavoriteItems.Clone(),
		withFavoriteTags:  _q.withFavoriteTags.Clone(),
...
func (_q *UserQuery) WithFavoriteItems(opts ...func(*MetaQuery)) *UserQuery {
	query := (&MetaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFavoriteItems = query
	return _q
}
...
func (_q *UserQuery) WithFavoriteTags(opts ...func(*TagQuery)) *UserQuery {
	query := (&TagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFavoriteTags = query
	return _q
}
...
func (_q *UserQuery) WithProgress(opts ...func(*ProgressQuery)) *UserQuery {
	query := (&ProgressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [4]bool{
			_q.withFavoriteItems != nil,
			_q.withFavoriteTags != nil,
	...
	if query := _q.withFavoriteItems; query != nil {
		if err := _q.loadFavoriteItems(ctx, query, nodes,
			func(n *User) { n.Edges.FavoriteItems = []*Meta{} },
			func(n *User, e *Meta) { n.Edges.FavoriteItems = append(n.Edges.FavoriteItems, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFavoriteTags; query != nil {
		if err := _q.loadFavoriteTags(ctx, query, nodes,
...
func (_q *UserQuery) loadFavoriteItems(ctx context.Context, query *MetaQuery, nodes []*User, init func(*User), assign func(*User, *Meta)) error {
...
func (_q *UserQuery) loadFavoriteTags(ctx context.Context, query *TagQuery, nodes []*User, init func(*User), assign func(*User, *Tag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
...
func (_q *UserQuery) loadProgress(ctx context.Context, query *ProgressQuery, nodes []*User, init func(*User), assign func(*User, *Progress)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
